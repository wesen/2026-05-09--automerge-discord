...
Using automerge means storing your data in automerge [documents](https://automerge.org/docs/reference/concepts/#documents). Documents have [URL](https://automerge.org/docs/reference/concepts/#document-urls) s which you can use to share or request documents with/from other peers using a [repository](https://automerge.org/docs/reference/concepts/#repositories). Repositories give you [`DocHandle`](https://automerge.org/docs/reference/concepts/#dochandles) s which you use to make changes to the document and listen for changes from other peers.

Automerge as used in javascript applications is actually a composition of two libraries. [`automerge-repo`](https://www.npmjs.com/package/@automerge/automerge-repo) which provides the networking and storage plumbing, and [`automerge`](https://www.npmjs.com/package/@automerge/automerge) which provides the CRDT implementation, a transport agnostic [sync protocol](https://automerge.org/docs/reference/concepts/#sync-protocol), and a compressed [storage format](https://automerge.org/docs/reference/concepts/#storage-format) which `automerge-repo` uses to implement various networking and storage plugins.
...
Like a JSON object, an automerge document is a map from strings to values, where the values can be maps, arrays, or simple types like strings or numbers. See the [data model](https://automerge.org/docs/reference/documents/) section for more details.

Like a git repository, an automerge document has a history made up of commits. Every time you make a change to a document you are adding to the history of the document. The combination of this history and some rules about how to handle conflicts means that any two automerge documents can always be merged. See [merging](https://automerge.org/docs/reference/under-the-hood/merge-rules) for the gory details.
