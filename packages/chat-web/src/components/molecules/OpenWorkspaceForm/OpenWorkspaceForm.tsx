...
export function OpenWorkspaceForm({ defaultSyncUrl, onOpen }: OpenWorkspaceFormProps) {
  ...
  return (
    <form data-widget="autodisco" data-part="open-workspace-form" onSubmit={submit}>
      <MacTextField label="Automerge URL" value={workspaceDocUrl} onChange={(event) => setWorkspaceDocUrl(event.currentTarget.value)} helperText="Paste a workspace URL from another session to join it." />
